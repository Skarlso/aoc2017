...
class Direction {
    ...
    public function setReversed(Direction $direction)
    {
        $this->reversed = $direction;
    }

    public function getReversed(): Direction
    {
...
$LEFT->setLeft($DOWN);
$LEFT->setRight($UP);
$LEFT->setReversed($RIGHT);

$RIGHT->setLeft($UP);
$RIGHT->setRight($DOWN);
$RIGHT->setReversed($LEFT);
...
function turn(Direction $direction, int $x, int $y): Direction {
    ...
    if ($grid[$y][$x] === "#") {
        ...
        $direction = $direction->getReversed();
    }
    return $direction;
}
